...
function distributeMinutes(subjects, priorities, totalMinutes) {
  const weightByPriority = { high: 3, medium: 2, low: 1 };

  let totalWeight = 0;
  subjects.forEach(subject => {
    totalWeight += weightByPriority[priorities[subject]];
  });

  const allocated = {};

  subjects.forEach(subject => {
    const weight = weightByPriority[priorities[subject]];
    let minutes = Math.round((weight / totalWeight) * totalMinutes);

    minutes = Math.max(15, minutes);

    allocated[subject] = minutes;
  });
...
function savePlan() {
  lastPlan = JSON.parse(JSON.stringify(currentPlan));
...
function renderPlan() {
  if (!currentPlan.length) {
    if (isWeeklyView) {
  planOutput.innerHTML = currentPlan.map(dayObj => {
    return `<div class="plan-day">
      <h3>Day ${dayObj.day}</h3>
      ${dayObj.tasks.map(t => `<p>${t.subject} (${formatMinutes(t.minutes)})</p>`).join("")}
    </div>`;
  }).join("");
  return;
}
...
function updateInsights() {
  ...
  tasks.forEach(t => {
    total += t.minutes;
    subjectMap[t.subject] = (subjectMap[t.subject] || 0) + t.minutes;
  });

  const top = Object.keys(subjectMap).reduce((a, b) =>
    subjectMap[a] > subjectMap[b] ? a : b, "-");

  document.getElementById("totalTime").textContent =
    "Total Time: " + Math.round(total / 60) + " hrs";
...
function updateGoal() {
  const goal = Number(document.getElementById("dailyGoal").value) * 60;
  const done = flattenTasks().filter(t => t.done)
...
plannerForm.addEventListener("submit", function (event) {
    planOutput.innerHTML = "<p class='placeholder'>Generating plan...</p>";
...
document.addEventListener("keydown", function (event) {
  ...
  if (
    ...
    active.tagName !== "INPUT" &&   // ✅ allow typing in inputs
